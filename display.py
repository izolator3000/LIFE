...
class Display:
    ...
    def __transform_coords(self, coords):
        ...
        for coord in coords:
            outptut.append(
                Coords(coord.x * self.cell_size + self.width / 2 + self.__offset.x,
                       self.height / 2 - (coord.y * self.cell_size) - self.__offset.y))
        return outptut

    def __get_cuurent_frame_cells(self, clock):
        self.__timer += clock.tick()
        if self.__timer < self.one_frame_delay:
            coords = self.field.get_current_generation()
        else:
            coords = self.field.get_new_generation()
            self.__timer = 0
        return coords
    ...
    def __offset_check(self):
        ...
        if coord.x > self.width - bound_width:
            res.x = -1
        if coord.x < bound_width:
            res.x = 1
        if coord.y < bound_height:
            res.y = -1
        if coord.y > self.height - bound_height:
            res.y = 1
    ...
    def draw(self):
        ...
        while running:
            ...
            coords = self.__get_cuurent_frame_cells(update_clock)
